	...
	return (
		<li>
			<Link href={navElement.path}>
				<a
					className={`cursor-pointer px-4 font-semibold uppercase group-hover:text-primary ${
						!bgTransparent || scrolled ? 'text-primary' : 'text-white'
					}`}
				>
					{navElement.title}
				</a>
...
export const Navbar: FC<props> = ({ bgTransparent }) => {
	...
	const [mobileMenuOpen, setMovilMenuOpen] = useState(false);
	...
	return (
		<Fragment>
			<div
				className={` fixed top-0 bottom-0 z-20 bg-black transition-opacity duration-500 ${
					mobileMenuOpen ? 'w-full opacity-50' : 'w-0 opacity-0'
				}`}
			></div>
			<div
				className={`fixed top-0 bottom-0 z-30 h-full bg-white transition-all duration-500 ${mobileMenuOpen ? '' : ''}`}
				style={{ width: '767px', maxWidth: '90%', left: mobileMenuOpen ? '0' : '-767px' }}
			>
			...
			<div id='Navbar' className={`z-20 h-full ${scrolled && 'drop-shadow-lg'}`}>
				<nav
